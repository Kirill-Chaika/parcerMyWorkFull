...
async function massOllInk(mass, querySelect ,priceSelector, whoIs) {
  for (let i = 0; i < mass.length; i += 1) {
    await page.goto(mass[i]);
    const n = await page.$("#txt");

    let arr3 = await page.evaluate(() => {
      let text2 = document.querySelector(querySelect).innerText;
      if (document.querySelector(priceSelector) != null) {
        return (
          text2 + whoIs + document.querySelector(priceSelector).innerText
        );
      } else {
        return text2;
      }
...
async function f() {
  ...
  for (let i = 0; i < arrLinkGro14andPlus.length; i += 1) {
    ...
    let arr3 = await page.evaluate(() => {
      ...
      if (document.querySelector("bdi") != null) {
        return (
          text2 + "Gro: " + document.querySelector("bdi").innerText
        );
      } else {
        return text2;
      }
  ...
  massOllInk(arrLinkGro14andPlus,"h1","bdi","Gro: ")
  ...
  for (let i = 0; i < arrLinkGro15Pro.length; i += 1) {
    await page.goto(arrLinkGro15Pro[i]);
    const n = await page.$("#txt");
  ...
  for (let i = 0; i < arrLinkGro15ProMax.length; i += 1) {
    await page.goto(arrLinkGro15ProMax[i]);
    const n = await page.$("#txt");
